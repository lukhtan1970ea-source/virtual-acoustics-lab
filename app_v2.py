...
st.set_page_config(page_title="Virtual Lab v2: Young's Modulus", layout="wide")

st.title("🔬 Virtual Acoustics Lab (Version 2)")
...
with col1:
    st.header("⚙️ Controls & Apparatus Scheme")
    material = st.selectbox("Select Rod Material:", list(MATERIALS.keys()))
    mat_data = MATERIALS[material]
    
    st.info("📏 **Rod Specifications:**\n* Length (L): 0.500 m\n* Diameter (d): 15.0 mm")
    
    # --- ВИЗУАЛЬНАЯ СХЕМА ЛАБОРАТОРНОЙ УСТАНОВКИ В ИНТЕРФЕЙСЕ ---
    st.write("**EXPERIMENTAL APPARATUS SCHEME:**")
    st.code(
f"""
  [ SIGNAL GENERATOR ]              [ DIGITAL OSCILLOSCOPE ]
          ...
          ▼                                    │
   ┌──────────────┐                     ┌──────────────┐
   │ Exciter Coil │                     │ Piezo Sensor │
   │ (Piezo-Drive)│                     │  (Receiver)  │
   └──────┬───────┘                     └───────▲──────┘
          │ (Mechanical wave)                   │ (Signal pickup)
          ▼                                     │
   ┌──────────────┬──────────█──────────┬──────────────┐
  ( )            ( )       CLAMP       ( )            ( ) <-- Free Rod Ends
   └──────────────┴──────────█──────────┴──────────────┘
...
""", language="text")
    ...
    st.markdown("""
    **PHYSICAL PRINCIPLE:**
    * The **Generator** sends an electrical sine wave to the **Exciter Coil**.
    * The Exciter pushes the left end, creating a longitudinal elastic wave inside the **Metal Rod**.
    * The wave reflects from free ends, forming a **Standing Wave** with a Node at the central clamp.
    * The **Piezo Sensor** captures mechanical vibrations at the right end and sends them to the **Oscilloscope**.
    """)

# Isolated Interactive Fragment
@st.fragment
def run_experiment_block_v2(selected_material, physics_data):
    ...
    # Отрисовка стандартного Matplotlib
...
run_experiment_block_v2(material, mat_data)
